...
csvString_to_data2 : String -> List Wines
csvString_to_data2 csvRaw =
    Csv.parse csvRaw
        |> Csv.Decode.decodeCsv decodeWines
        |> Result.toMaybe
        |> Maybe.withDefault []

type alias Wines =
    {name : String, price : Float, year : Float}

decodeWines : Csv.Decode.Decoder (Wines -> a) a
decodeWines =
    Csv.Decode.map Wines
        ( Csv.Decode.field "name" Ok
            |> Csv.Decode.andMap (Csv.Decode.field "price"  (String.toFloat >> Result.fromMaybe "error parsing string"))
