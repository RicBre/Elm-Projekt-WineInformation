...
var $author$project$BaumHierarchie$GotFlare = function (a) {
	return {$: 'GotFlare', a: a};
};
...
function $author$project$BaumHierarchie$cyclic$jsonDekodierung() {
	return A3(
			...
			A2(
				...
				$elm$json$Json$Decode$list(
					$elm$json$Json$Decode$lazy(
						function (_v1) {
							return $author$project$BaumHierarchie$cyclic$jsonDekodierung();
						})))));
}
try {
	var $author$project$BaumHierarchie$jsonDekodierung = $author$project$BaumHierarchie$cyclic$jsonDekodierung();
	$author$project$BaumHierarchie$cyclic$jsonDekodierung = function () {
		return $author$project$BaumHierarchie$jsonDekodierung;
	};
} catch ($) {
	throw 'Some top-level definitions from `BaumHierarchie` are causing infinite recursion:\n\n  ┌─────┐\n  │    jsonDekodierung\n  └─────┘\n\nThese errors are very tricky, so read https://elm-lang.org/0.19.1/bad-recursion to learn how to fix it!';}
var $author$project$BaumHierarchie$init = function (_v0) {
	return _Utils_Tuple2(
		{
			baum: A2($alex_tan$elm_tree_diagram$TreeDiagram$node, '', _List_Nil),
			errorMsg: 'Loading ...'
		},
		$elm$http$Http$get(
			{
				expect: A2($elm$http$Http$expectJson, $author$project$BaumHierarchie$GotFlare, $author$project$BaumHierarchie$jsonDekodierung),
				url: 'https://raw.githubusercontent.com/RicBre/Elm-Projekt-WineInformation/main/Daten/AufbereiteteDaten/WineInformationGeoKleinKlein.json'
			}));
};
...
var $author$project$BaumHierarchie$update = F2(
	function (msg, model) {
		if (msg.a.$ === 'Ok') {
			var newTree = msg.a.a;
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{baum: newTree, errorMsg: 'No Error'}),
...
var $author$project$BaumHierarchie$zeichneKnoten = function (n) {
	return A2(
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								A2($elm_community$typed_svg$TypedSvg$Types$Translate, -5.5, -20.5),
								A3($elm_community$typed_svg$TypedSvg$Types$Rotate, 50.0, 0.0, 0.0)
							])),
...
var $author$project$BaumHierarchie$zeichneLinie = function (_v0) {
	var zielX = _v0.a;
	var zielY = _v0.b;
	return A2(
		...
		_List_fromArray(
			[
				...
				$elm_community$typed_svg$TypedSvg$Attributes$InPx$x2(zielX),
				$elm_community$typed_svg$TypedSvg$Attributes$InPx$y2(zielY),
				$elm_community$typed_svg$TypedSvg$Attributes$stroke(
				$elm_community$typed_svg$TypedSvg$Types$Paint($avh4$elm_color$Color$gray))
			]),
		_List_Nil);
};
var $author$project$BaumHierarchie$view = function (model) {
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
			[
				A4($alex_tan$elm_tree_diagram$TreeDiagram$Svg$draw, $alex_tan$elm_tree_diagram$TreeDiagram$defaultTreeLayout, $author$project$BaumHierarchie$zeichneKnoten, $author$project$BaumHierarchie$zeichneLinie, model.baum)
			]));
};
var $author$project$BaumHierarchie$main = $elm$browser$Browser$element(
	{
		init: $author$project$BaumHierarchie$init,
		subscriptions: function (m) {
			return $elm$core$Platform$Sub$none;
		},
		update: $author$project$BaumHierarchie$update,
		view: $author$project$BaumHierarchie$view
	});
_Platform_export({'BaumHierarchie':{'init':$author$project$BaumHierarchie$main(
